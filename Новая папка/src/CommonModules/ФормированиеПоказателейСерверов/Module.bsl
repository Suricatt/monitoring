...
// 	Описание
	Функция ИницилизироватьДанныеСерверов() Экспорт
		...
		ТаблицаСерверов.Колонки.Добавить("АдресСервера");
		...
		ТаблицаСерверов.Колонки.Добавить("ПутьКПапкеZabbix");

		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ
		|	РабочиеСервера.Ссылка,
		|	РабочиеСервера.Наименование КАК ИмяСервера,
		|	РабочиеСервера.Адрес КАК АдресСервера,
		|	РабочиеСервера.ЛогинАдминистратора,
		|	РабочиеСервера.ПарольАдминистартора,
		|	РабочиеСервера.ПутьКПапкеZabbix
		|ИЗ
...
// 	Описание
	...
	Процедура ДобавитьДанныеМониторинга(ПараметрыДанных, ДанныеСервера)
		...
		Агент = Соединение.ConnectAgent(ДанныеСервера.АдресСервера);
